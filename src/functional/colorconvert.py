"""
Copied from https://github.com/TheZino/pytorch-color-conversions
"""
import torch
...
def rgb2ycbcr(rgb: torch.Tensor) -> torch.Tensor:
    ...
    return _mul(_RGB_TO_YCBCR.to(rgb.device), rgb) + _YCBCR_OFF.to(rgb.device)


def ycbcr2rgb(ycbcr: torch.Tensor) -> torch.Tensor:
    """YCbCr to sRGB conversion."""
    rgb = _mul(torch.inverse(_RGB_TO_YCBCR.to(ycbcr.device)), ycbcr - _YCBCR_OFF.to(ycbcr.device))
    return rgb
...
def _convert(matrix: torch.Tensor, arr: torch.Tensor) -> torch.Tensor:
...
def get_xyz_coords(illuminant: str, observer: str) -> torch.Tensor:
    ...
    -------
    (x, y, z) : Tensor
        A Tensor with 3 elements containing the XYZ coordinates of the given
        illuminant.
    Raises
    ------
    ValueError
...
def xyz2lab(xyz: torch.Tensor, illuminant: str = "D65", observer: str = "2") -> torch.Tensor:
...
def rgb2lab(rgb: torch.Tensor, illuminant: str = "D65", observer: str = "2") -> torch.Tensor:
...
def lab2xyz(lab: torch.Tensor, illuminant: str = "D65", observer: str = "2") -> torch.Tensor:
    arr = lab.clone()
...
def xyz2rgb(xyz: torch.Tensor) -> torch.Tensor:
...
def lab2rgb(lab: torch.Tensor, illuminant: str = "D65", observer: str = "2") -> torch.Tensor:
