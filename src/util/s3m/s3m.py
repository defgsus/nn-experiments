...
# [row][channel]
PatternType = List[List[ChannelRow]]
...
class S3m:
    ...
    def _read_instruments(self, buf: bytes):
        ...
        for ptr in self.header.ptrinstruments:
            ...
            if inst.type == 0:  # empty instrument
                ...
                if inst.sample_ptr:
                    inst.sample_data = np.frombuffer(
                        buf[inst.sample_ptr * 16: inst.sample_ptr * 16 + inst.length],
                        dtype=np.uint8
                    ) - 128.
    ...
    def _read_patterns(self, buf: bytes):
        ...
        for ptr in self.header.ptrpatterns:
            ...
            for i in range(64):
                row: List[ChannelRow] = [ChannelRow() for _ in range(32)]
                while True:
                    try:
