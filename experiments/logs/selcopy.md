...
# 2024-12-14 Efficiently solving the Selective Copy Problem with a Very Small Language Model
...
*Selective Copy* problem with a very very small, fast and general network.

The model has an input-embedding which is fed by raw byte values of texts. 
It's nasty in reversing the model's output put keeps things otherwise simple and easy.

The *Selective Copy* Dataset is the (almost) endless stream of combinations of letters, 
...
but instead of a `?`, the mask byte value is actually zero. So, the model get's this sequence of
...
I mean, really! A lot of people have access, through their companies, to LARGE
