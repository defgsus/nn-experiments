...
class TeletextIterableDataset(IterableDataset):

    def __init__(self, filename: Union[str, Path] = DATASET_PATH / "teletext.ndjson.gz"):
        super().__init__()
        self.filename = Path(filename)

    def __iter__(self):
        for entry in iter_ndjson(self.filename):
            text = entry.pop("text")
            yield text, entry


class TeletextMatrixIterableDataset(IterableDataset):
